...
namespace Zero
{
    ...
    public class ILBridge : ASingletonMonoBehaviour<ILBridge>
    {
        ...
        public void Startup()
        {
            ...
            {
                Debug.Log(AppDomain.CurrentDomain.GetAssemblies()[i]);
            }
            var script = AppDomain.CurrentDomain.GetAssemblies().First(a => a.GetName().Name == "Scripts");
            Debug.Log(script);
            //使用Assembly
            iLWorker = new AssemblyILWorker(script);
        }
        ...
        public void Startup(byte[] dllBytes, byte[] pdbBytes = null)
        {
