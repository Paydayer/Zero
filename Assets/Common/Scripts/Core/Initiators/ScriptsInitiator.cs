...
namespace Zero
{
    internal class ScriptsInitiator : BaseInitiator
    {
        ...
        /// 热更DLL路径
        /// </summary>
        private string HotDllPath => FileUtility.CombinePaths(Runtime.localResDir, ZeroConst.DLL_DIR_NAME, ZeroConst.DLL_FILE_NAME + ".dll.bytes");
        ...
        void LoadDllBytes(out byte[] dllBytes, out byte[] pdbBytes)
        {
            ...
            if (Runtime.IsHotResEnable)
            {
                ...
                if (File.Exists(dllPath))
                {
                    ...
                    // string pdbPath = FileUtility.CombinePaths(Runtime.localResDir, ZeroConst.DLL_DIR_NAME + ".bytes", ZeroConst.DLL_FILE_NAME + ".pdb");
